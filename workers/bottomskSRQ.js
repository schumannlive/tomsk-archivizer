...
console.log('SRQ worker started')
...
schedule.scheduleJob(rulesSpawn, function(){
    ...
    const ls = spawn(process.argv[0], ['./workers/slicerSRQ.js'], {
        detached: true
      });

    ls

    console.log('SRQ ' + today + ' sliced')
...
function dlpic(){
    ...
    var request = http.get("http://sosrff.tsu.ru/new/srq.jpg",  function(response) {
        if (response.statusCode === 200) {
            var file =  fs.createWriteStream("./schumann_archive_SRQ/" + today + ".jpg");
             response.pipe(file).on('finish', () => {
                console.log('SRQ ' + today + ' downloaded')
              });
        }
    });
...
function rename(){
    ...
    fs.unlink('./schumann_archive_SRQ/section-1.jpg', function (err) {
    });
    fs.unlink('./schumann_archive_SRQ/section-3.jpg', function (err) {
     });
    fs.unlink('./schumann_archive_SRQ/section-4.jpg', function (err) {
    });
    fs.unlink('./schumann_archive_SRQ/section-5.jpg', function (err) {
    });

    fs.rename('./schumann_archive_SRQ/section-2.jpg','./schumann_archive_singulardays_SRQ/' + today + '.jpg', function (err) {
        if (err) throw err;
        else console.log('SRQ ' + today + ' deleted and renamed');
    });
...
let inputFile  = './schumann_archive_singulardays_SRQ/' + today + '.jpg';
let outputFile = './schumann_archive_singulardays_SRQ/' + today + '_stretched.jpg';

sharp(inputFile).resize({ height: 340, width: 481, fit: 'fill'}).toFile(outputFile)
    .then(function(newFileInfo) {
        console.log('SRQ ' + today + " resized");
    })
    .catch(function(err) {
