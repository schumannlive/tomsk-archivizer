...
console.log('SRA worker started')
...
schedule.scheduleJob(rulesSpawn, function(){
    ...
    const ls = spawn(process.argv[0], ['./workers/slicerSRA.js'], {
        detached: true
      });

    ls

    console.log('SRA ' + today + ' sliced')
...
function dlpic(){
    ...
    var request = http.get("http://sosrff.tsu.ru/new/sra.jpg",  function(response) {
        if (response.statusCode === 200) {
            var file =  fs.createWriteStream("./schumann_archive_SRA/" + today + ".jpg");
             response.pipe(file).on('finish', () => {
                console.log('SRA ' + today + ' downloaded')
              });
        }
    });
...
function rename(){
    ...
    fs.unlink('./schumann_archive_SRA/section-1.jpg', function (err) {
    });
    fs.unlink('./schumann_archive_SRA/section-3.jpg', function (err) {
     });
    fs.unlink('./schumann_archive_SRA/section-4.jpg', function (err) {
    });
    fs.unlink('./schumann_archive_SRA/section-5.jpg', function (err) {
    });

    fs.rename('./schumann_archive_SRA/section-2.jpg','./schumann_archive_singulardays_SRA/' + today + '.jpg', function (err) {
        if (err) throw err;
        else console.log('SRA ' + today + ' deleted and renamed');
    });
...
let inputFile  = './schumann_archive_singulardays_SRA/' + today + '.jpg';
let outputFile = './schumann_archive_singulardays_SRA/' + today + '_stretched.jpg';

sharp(inputFile).resize({ height: 340, width: 481, fit: 'fill'}).toFile(outputFile)
    .then(function(newFileInfo) {
        console.log('SRA ' + today + " resized");
    })
    .catch(function(err) {
